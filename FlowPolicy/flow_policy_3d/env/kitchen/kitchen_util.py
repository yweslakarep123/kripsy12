...
def parse_mjl_logs(read_filename, skipamount):
    ...
    # Some logs are truncated (payload not aligned to 4 bytes or recsz). Keep only
    # complete records so loading does not fail on slightly corrupt tail bytes.
    usable_bytes = len(payload) - (len(payload) % 4)
    num_floats = usable_bytes // 4
    nrec = num_floats // recsz
    if nrec == 0:
        raise ValueError(
            f"MJL corrupt or too short ({len(payload)} payload bytes, recsz={recsz}): "
            f"{read_filename}"
        )
    num_floats = nrec * recsz
    dat = np.asarray(
        struct.unpack(
            str(num_floats) + "f",
            payload[: num_floats * 4],
        )
    )
    dat = np.reshape(dat, (nrec, recsz)).T
