...
def take_last_n(x, n):
    ...
    if isinstance(x[0], torch.Tensor):
        ...
        try:
            ...
            # Ragged per-step values (e.g. variable-length episode_task_completions lists)
                    ...
                    sig.append(
                        {
                            ...
                            "kind": "list",
                            "len": len(t),
                            "head": [type(e).__name__ for e in t[:5]],
                        }
                    )
                else:
                    sig.append({"i": i, "kind": type(t).__name__})
            # #region agent log
            try:
                p = {
                    "sessionId": "74ea2d",
                    "location": "multistep_wrapper.py:dict_take_last_n",
                    "message": "take_last_n ValueError",
                    "data": {"key": key, "n": n, "tail_sig": sig},
